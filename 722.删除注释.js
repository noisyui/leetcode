    ...
    for (let i = 0; i < source.length; i++) {
        ...
        if (inMultiComment) {
            ...
            if (endIndex !== -1) {
                ...
                source[i--] = finalLine + line.slice(endIndex + 2);
                continue;
            }
        } else {
            let inlineIndex = line.indexOf('//');
            let beginIndex = line.indexOf('/*');
            if (inlineIndex !== -1 && (beginIndex === -1 || inlineIndex < beginIndex)) {
                finalLine = line.slice(0, inlineIndex);
            } else if (beginIndex !== -1 && (inlineIndex === -1 || beginIndex < inlineIndex)) {
                let endIndex = line.indexOf('*/');
                while (endIndex !== -1 && endIndex < beginIndex + 2) {
                    endIndex = line.indexOf('*/', endIndex + 1);
                }
                if (endIndex === -1) {
                    inMultiComment = true;
                    finalLine = line.slice(0, beginIndex);
                } else {
                    source[i--] = line.slice(0, beginIndex) + line.slice(endIndex + 2);
                    continue;
                }
            } else {
                finalLine = line;
        ...
        if (finalLine.length && !inMultiComment) {
